defmodule Lyricsmarkov.Timeline do
  @moduledoc """
  ...
  def inc_reposts(%Post{id: id}) do
    {1, [post]} =
      from(p in Post, where: p.id == ^id, select: p)
    |> Repo.update_all(inc: [num_shares: 1])

    broadcast({:ok, post}, :post_updated)
  end

  def subscribe do
    Phoenix.PubSub.subscribe(Lyricsmarkov.PubSub, "posts")
  end

  defp broadcast({:error, _reason} = error, _event), do: error
  defp broadcast({:ok, post}, event) do
    Phoenix.PubSub.broadcast(Lyricsmarkov.PubSub, "posts", {event, post})
    {:ok, post}
  end
end
